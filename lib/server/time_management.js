...
//WORKING NI TANAN SA GUSTO NKO NA OUTPUT. e update kay mag focus sa device time
...
async function syncTimeSlot(db, childId, timeSlots, deviceTime = null) {
  ...
  for (const slot of timeSlots) {
    ...
    // Ensure that we either use the provided slot_identifier or create a new one
    const slotIdentifier = slot_identifier ? new ObjectId(slot_identifier) : new ObjectId();

    // Construct slot start and end times based on the provided deviceTime (or server time)
    const slotStartTime = new Date(new Date(now).toDateString() + ' ' + start_time); // Create date using device's time
    const slotEndTime = new Date(new Date(now).toDateString() + ' ' + end_time); // Use device's time for end time

    let remainingTime;

    // Calculate remaining time based on device time
    ...
    // Log for each slot with remaining time and correct slot_identifier
