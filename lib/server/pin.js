...
module.exports = (db) => {
  ...
  router.post('/set-pin', async (req, res) => {
    ...
    try {
      // Retrieve the existing PIN document
      const childPinDoc = await db.collection('child_pin').findOne({ childId: childId });

      if (childPinDoc) {
        // If a document exists, check if the PIN is different
        const hashedPin = childPinDoc.pin;
        const isMatch = await bcrypt.compare(pin, hashedPin);

        if (isMatch) {
          return res.status(400).json({ message: 'This PIN is already set for the child.' });
        }
      }
      ...
      await db.collection('child_pin').findOneAndUpdate(
      ...
      console.log(`PIN updated for childId: ${childId}`);
  ...
  router.post('/verify-pin', async (req, res) => {
    ...
    console.log(`Received childId: ${childId}, PIN for verification: ${pin}`);

    // Check if the childId and pin are valid
    if (!childId || typeof childId !== 'string') {
        return res.status(400).json({ message: 'Invalid childId format. Must be a non-empty string.' });
    }
    ...
    try {
        ...
        if (!childPinDoc) {
            console.log(`No PIN found for childId: ${childId}`);
            return res.status(404).json({ message: 'No PIN set for this child.' });
        }
        ...
        if (isMatch) {
            console.log(`PIN verified successfully for childId: ${childId}`);
            return res.status(200).json({ message: 'PIN verified successfully' });
        } else {
            console.log(`Incorrect PIN entered for childId: ${childId}`);
